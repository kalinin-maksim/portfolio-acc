...
@Service
@Log4j2
public class LoanEventProcessorService extends GenericProcessorService {

    private final IntegrationConfiguration integrationConfiguration;
    private final RuleMapper ruleMapper;
    private final EntryMapper entryMapper;

    public LoanEventProcessorService(MessageService messageService, EntryService entryService, DroolsRuleRepository droolsRuleRepository, IntegrationConfiguration integrationConfiguration, RuleMapper ruleMapper, EntryMapper entryMapper) {
    ...
    protected String getTopicIn() {
        return integrationConfiguration.getTopicIn(ExternalSystem.LOANS.toString());
    }

    protected UnSafeFunction<MessageDto, String> processing(RuleEngine ruleEngine) {
        return message -> {
            var event = getObject(message, LoanMessageDTO.class);
